...
object BaseUtils {
    ...
    private fun getGridCards(resources: Resources): List<ItemCard> {
        val on7 = createItemCard(
            resources, R.string.on7_titletext, R.string.on7_image_url,
            R.string.on7_subtext, R.string.on7_summarytext
        )

        val note5 = createItemCard(
            resources, R.string.note5_titletext, R.string.note5_image_url,
            R.string.note5_subtext, R.string.note5_summarytext
        )

        val pixel = createItemCard(
            resources, R.string.pix_titletext, R.string.pix_image_url,
            R.string.pix_subtext, R.string.pix_summarytext
        )

        val iphone6 = createItemCard(
            resources, R.string.i6_titletext, R.string.i6_image_url,
            R.string.i6_subtext, R.string.i6_summarytext
        )

        val moto = createItemCard(
            resources, R.string.moto_titletext, R.string.moto_image_url,
    ...
    fun getCards(resources: Resources, type: Int): List<ItemCard>? {
        ...
        when (type) {
            ...
            TYPE_GRID, TYPE_SECOND_GRID, TYPE_GRID_ZOOM -> itemCards = getGridCards(resources)
            else -> itemCards = null
        }
    ...
    fun getDemoConfiguration(configurationType: Int, context: Context): DemoConfiguration? {
        ...
        when (configurationType) {
            ...
            TYPE_GRID_ZOOM -> {
                demoConfiguration = DemoConfiguration()
                demoConfiguration.styleResource = R.style.AppThemeGrid
                demoConfiguration.layoutResource = R.layout.activity_zoom
